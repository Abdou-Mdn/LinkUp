 ...
 * Displays the tertiary button type  
 * used for less important actions
 * the button's colors are transparent background with text colors, on hover becomes secondary colors 
  ...
  return (
    <button
        ...
        className={`outline-0 text-center rounded-4xl transition-all font-medium border-1 
            ${ isDisabled ? 'border-gray-500 bg-gray-500 text-inverted cursor-not-allowed' : 'border-light-txt text-light-txt dark:border-dark-txt dark:text-dark-txt hover:border-secondary hover:text-secondary cursor-pointer'}
            ${className}
        `}
    >
        {loading ? (
